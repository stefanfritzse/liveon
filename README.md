...
The interactive coach surfaces a POST endpoint at `/api/ask` that accepts a
JSON payload containing a `question` string. The service retrieves matching
Firestore snippets, prompts the configured language model, and returns a
structured response containing:

- `answer` – The coach guidance generated by the LLM.
- `disclaimer` – A mandatory safety reminder appended to every reply.
- `citations` – The Firestore snippets referenced in the answer.

Configure the backing model by exporting the following environment
variables (defaults reflect the deterministic local responder used during
development):

| Variable | Default | Description |
| --- | --- | --- |
| `LIVEON_COACH_MODEL` | `local` | Select the responder backend (`local`, `vertex`, or `openai`). |
| `LIVEON_COACH_VERTEX_MODEL` | `chat-bison` | Vertex AI chat model invoked when the provider is `vertex`. |
| `LIVEON_VERTEX_LOCATION` | _unset_ | Optional Vertex AI region override (e.g. `us-central1`). |
| `LIVEON_COACH_OPENAI_MODEL` | `gpt-4o-mini` | OpenAI chat model used when the provider is `openai`. |
| `LIVEON_MODEL_TEMPERATURE` | `0.2` | Sampling temperature applied to the configured LLM. |
| `LIVEON_MODEL_MAX_OUTPUT_TOKENS` | `1024` | Maximum tokens returned by the LLM. |

When Firestore has no relevant context the service instructs the model to
provide general educational guidance and to reiterate that medical advice
  ...
  "citations": [
    {
      ...
      "snippet": "Maintain a regular sleep schedule and wind-down routine.",
      "article_id": "sleep-123",
      "score": 0.87,
      "published_at": "2024-06-15T08:00:00+00:00"
    }
  ]
}
```
