...
The web layer fetches data from the SQLite database via the content
repository. If the database is not available during local development, the
application falls back to in-memory sample data so that the UI remains
accessible.
...
curl -X POST http://localhost:8000/api/ask \
  ...
  -d '{"question": "How can I improve my sleep quality?"}'
```
...
```json
{
  "answer": "Aim for a consistent sleep and wake schedule...",
  "disclaimer": "This conversation is for educational purposes only and is not a substitute for professional medical advice..."
}
```
...
The application uses a content system for articles and coaching tips, with
the data access layer and domain models located in `app/services/` and
`app/models/` respectively.
...
to the repository, treating content as code. For dynamic SQLite-backed
deployments, the `SQLitePublisher` writes articles directly to the
database, deduplicating updates so rerunning the pipeline does not spam
users with repeated posts. The corresponding tests in
`app/tests/test_publisher.py` cover both workflows: the Git path
initialises a temporary repository to validate commits, while the SQLite
path ensures articles are stored with deterministic slugs and timestamps.
This lays the groundwork for integrating the multi-agent workflow with
GitOps tooling and the live SQLite content store in later milestones.
...
duplicates. Because the FastAPI views read directly from the database, any
new articles are automatically surfaced on the homepage and `/articles`
listing.
...
resulting article directly to the database so it appears on the FastAPI
frontend.
...
Run the FastAPI and coach unit tests locally before opening a pull request:

```bash
pytest app/tests/test_coach_agent.py app/tests/test_main_app.py
```
...
The app uses a local SQLite database for content storage. If the database
is not available, the app will display seeded placeholder data.

### Seeding the database

The `app/scripts/seed_content.py` script populates the database with a
starter article and tip. The script only inserts documents when the
`articles` or `tips` collection is empty, making it safe to run multiple
