...
def test_gather_normalizes_urls_when_deduplicating() -> None:
    feeds = [FeedSource(name="Longevity Research Digest", url="https://example.com/feed")]
    aggregator = LongevityNewsAggregator(feeds, fetcher=lambda url: URL_VARIANTS_FEED)

    result = aggregator.gather()

    assert len(result.items) == 1
    assert result.items[0].url == "https://example.com/articles/peptide-therapy"


def test_gather_deduplicates_entries_without_urls() -> None:
    feeds = [FeedSource(name="Longevity Research Digest", url="https://example.com/feed")]
    aggregator = LongevityNewsAggregator(feeds, fetcher=lambda url: MISSING_URL_FEED)

    result = aggregator.gather()

    assert len(result.items) == 1
    assert result.items[0].title == "Cellular senescence markers fall after therapy"
