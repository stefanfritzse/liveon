...
class _CollectionRef:
    """Duck-type of a Firestore collection, only used for its .id property."""
...
class LocalSQLiteContentRepository:
    ...
    def save_article(self, article: Article) -> Article:
        ...
        art_id = article.id or self._generate_id()
        article.id = art_id
        doc = article.to_dict()  # Use the model's own serializer
    ...
    def save_tip(self, tip: Tip) -> Tip:
        tip_id = tip.id or self._generate_id()
        tip.id = tip_id
        doc = tip.to_dict()
        title = doc.get("title")
    ...
    @property
    def articles(self) -> "_CollectionRef":
        """Return a reference to the 'articles' collection."""
        return self._article_collection

    @property
    def tips(self) -> "_CollectionRef":
        """Return a reference to the 'tips' collection."""
    ...
    def _row_to_article(self, row: sqlite3.Row | dict | None) -> Article:
        ...
        doc_id = row["id"] if isinstance(row, sqlite3.Row) else row.get("id", "")
        data_txt = row["data"] if isinstance(row, sqlite3.Row) else row.get("data")
        data = _json_or_none(data_txt) or {}
        return Article.from_document(_DictSnapshot(doc_id, data))
    ...
    def _row_to_tip(self, row: sqlite3.Row | dict | None) -> Tip:
        ...
        data_txt = row["data"] if isinstance(row, sqlite3.Row) else row.get("data")
        data = _json_or_none(data_txt) or {}
        return Tip.from_document(_DictSnapshot(doc_id, data))
...
def create_repository(**kwargs: Any) -> LocalSQLiteContentRepository:
    """Factory helper to create a repository."""
