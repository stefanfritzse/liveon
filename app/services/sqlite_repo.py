...
The repository exposes the following methods for retrieving and storing
application content:
...
- A small “snapshot” shim is used to call `Article.from_document(...)` / `Tip.from_document(...)`
...
# ---------------------------------------------------------------------------
# Small helpers that emulate the document snapshot and collection API
# ---------------------------------------------------------------------------

class _DictSnapshot:
    """Duck-typed version of a document snapshot for model.from_document(...)"""
...
# Factory to match other repository modules
