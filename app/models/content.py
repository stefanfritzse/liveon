...
#: A bullet the model left on its own list item, at either end. Hyphens and dashes are
#: only decoration when they lead and are followed by a space: "- longevity" is a bullet,
#: "annual check-ups" is a word and a DOI may legitimately end in one.
_LEADING_BULLET = re.compile(r"^[\s•▪▫●○◦‣·*]+|^[-–—]+\s+")
_TRAILING_BULLET = re.compile(r"[\s•▪▫●○◦‣·*]+$")


def _clean_text_value(value: str) -> str:
    """Strip a stored string of anything that is not readable text.

    Tags and the like are generated by a language model, and models emit control and
    formatting characters — zero-width spaces, stray bullets, private-use codepoints — that
    a browser draws as an empty box beside the word. Cleaning on read fixes what is already
    stored as well as what arrives next, which matters because the visible symptom is on a
    page rendered from rows written weeks ago.
    """

    # Cc control, Cf format (zero-width), Co private use, Cs surrogate: never legible text.
    stripped = "".join(
        character
    ...
    return _TRAILING_BULLET.sub("", _LEADING_BULLET.sub("", collapsed))


def _listify_strings(value: Any) -> list[str]:
    """Normalise a value into a list of non-empty, readable strings."""
